...
void 
close_socket(int sock, short which, client_conn_t *conn)
{
	...
	unsigned char *pkt)
{
	...
	initialize_iov(&conn->data, 
		sizeof(uint32_t) + hdr->caplen);

    LOG("Caplen %d\n", hdr->caplen);

    memcpy(conn->data.buf, &hdr->caplen, 
	    sizeof(uint32_t));

    memcpy(&conn->data.buf[sizeof(uint32_t)],
	    pkt, hdr->caplen);

    event_set(&conn->event, conn->sock, 
	    EV_WRITE, pcap_write, (void *)conn); 
    event_add(&conn->event, 0);

    /*
    event_set(&conn->event, conn->sock,
	    EV_READ | EV_PERSIST, close_socket, (void *)conn); 
    event_add(&conn->event, 0);
    */
}
...
pcap_driver(int sock, short which, client_conn_t *conn)
{
    ...
    pcap_dispatch(conn->descr, 1, 
	    (void *) pcap_write_init, (void *)conn);
}
...
pcap_event_init(client_conn_t *conn)
{
    ...
    event_set(&conn->pcap_event, conn->pcap_fd,
	    EV_READ | EV_PERSIST, pcap_driver, (void *)conn);
...
client_read_hdr_data(int sock, short which, client_conn_t *conn)
{
	...
	conn->start_header.dev = 
	    malloc(conn->start_header.dev_sz);
	...
	conn->start_header.bpf = 
	    malloc(conn->start_header.bpf_sz);
...
client_read_hdr(int sock, short which, client_conn_t *conn)
{
	...
	event_set(&conn->event, sock, EV_READ,
		(void *)client_read_hdr, conn);
	event_add(&conn->event, 0);
	return;
    }

    memcpy(&conn->start_header.dev_sz, 
	    &conn->data.buf[0], sizeof(uint8_t));
    memcpy(&conn->start_header.bpf_sz,
	    &conn->data.buf[1], sizeof(uint32_t));
    memcpy(&conn->start_header.snaplen,
	    &conn->data.buf[5], sizeof(uint16_t));
    memcpy(&conn->start_header.opts,
	    &conn->data.buf[7], sizeof(uint8_t));
...
server_driver(int sock, short which, void *args)
{
    ...
    csock = accept(sock, (struct sockaddr *)&addr, 
	    &addrlen);
